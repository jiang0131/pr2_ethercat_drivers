...
bool CAssembler::convertTokenToUnsignedValue(string str, unsigned int &value)
{
  char buf[32];
  bool success = (sscanf( translateConstant( str ).c_str(), "%X%30s", &value, buf) == 1);
  if (!success) {
    fprintf(stderr, "Error converting %s to value\n", str.c_str());
  }
  return success;
}
		...
		default:
			...
			case FETCH:
				if ( sourceLine.getColumn( offset + 3 ) == "(" ) {
					...
					unsigned int value ;
                                        if (!convertTokenToUnsignedValue(translateConstant(s3), value))
                                        {
...
// Exports memory list of hex values
bool CAssembler::exportMEM( string outputFile, string outputDir)
{
  int addr;
  unsigned int d ;
  CInstruction *instr ;
  ...
  if ( outfile == NULL ) {
    ...
    error( NO_LINE_NR , string( "Unable to open output file '" + outputFile + "'").c_str() ) ;
    return FALSE ;
  }
...
bool CAssembler::exportVHDL( string templateFileName, string outputFileName, string entityName, bool bVHDL)
{
	int addr, i, j, k, l, n ;
	...
	FILE * infile = fopen( templateFileName.c_str(), "r" ) ;
	if ( infile == NULL ) {
		error( NO_LINE_NR, string( "Unable to open template file '" + templateFileName + "'" ).c_str() ) ;
		return FALSE ;
	}
	...
	if ( outfile == NULL ) 
        {
	  error( NO_LINE_NR , string( "Unable to open output file '" + outputFileName + "'").c_str() ) ;
	  return FALSE ;
	}
		
	bool copy = false;
        int open_bracket_count=0; // count number of opening brackets '{' to allow nested verilog expressions
        std::string varname;    // use string instead of char[] buffer because it allows longer length
	int p = 0 ;
	int line, c ;
	while ( ( c = fgetc( infile ) ) != EOF ) {
          ...
          else //open_bracket_count == 0
          {
            ...
            {
              --open_bracket_count;
              if ( varname.compare(0,5, "INIT_") == 0 ) {
                sscanf( varname.c_str(), "INIT_%02X", &line ) ;
                if ( line >= 0 && line < 64 ) {
                  for( j = 31 ; j >= 0 ; j-- ) 
                    fprintf( outfile, "%02X", INIT[ j ][ line ] ) ;
                }
              } else if ( varname.compare(0,6,"INITP_") == 0 ) {
                sscanf( varname.c_str(), "INITP_%02X", &line ) ;
                if ( line >= 0 && line < 8 ) 
                  for( j = 31 ; j >= 0 ; j-- )
                    fprintf( outfile, "%02X", INITP[ j ][ line ] ) ;
              } else if ( varname == "name" ) {
                fputs( entityName.c_str(), outfile ) ;				
              } else if ( varname == "begin template" ) {
                copy = true ;
              } else {
                // verilog expressions inside { } are valid.  
                //For anything else, output data from inside expression
                fprintf( outfile, "{%s}", varname.c_str() ) ;
              }
            }
...
// DK (Apr 19 2011)
// export a file list mapping addresses to instructions 
bool CAssembler::exportHex(string outputFileName)
{
  int addr;
  unsigned int d, line;
  CInstruction *instr ;
  ...
  os << ";picoblaze binary" << endl
     << ";addr instruction  ; line number : disassmebly" << endl;

  for ( addr = 0 ; addr < 1024 ; addr++ ) 
  {
    instr = m_code->getInstruction( addr ) ;

    os << std::hex << std::setw(3) << std::setfill('0') << addr
    ...
    {
      os << instr->getHexCode()
         << std::dec << "     ; line " << instr->getSourceLine() << " :   ";
      instr->Print(os);
    }
    os << std::endl;
  }
